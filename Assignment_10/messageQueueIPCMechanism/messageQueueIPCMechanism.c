...
void receiveMessage(int, Message *, long);
void prepareMessage(Message *, int *, int, long);
...
void receiveMessage(int msgid, Message *msg, long msg_type)
{
    msgrcv(msgid, msg, sizeof(Message) - sizeof(long), msg_type, 0);
}

void prepareMessage(Message *msg, int *arr, int size, long msg_type)
{
...
void childProcess(int msgid)
{
    ...
    msg.msg_type = MSG_TYPE_TO_PARENT;
